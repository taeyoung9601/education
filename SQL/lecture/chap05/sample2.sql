...
SELECT
    -- 컬럼의 모호성이 없는 컬럼일지라도
    -- (즉, 어느 한쪽 테이블에만 있는 고유한 컬럼일지라도)
    -- 필요하다면, 컬럼의 모호성 제거 표기방식을 사용해도 문제는 없습니다.
    emp.last_name,

    -- 역시 컬럼의 모호성이 없는 부서명 컬럼이기 때문에,
    -- 모호성 제거 표기방식을 쓰지 않는게 오히려 정석입니다.
    department_name,

    -- 부서번호는 양쪽의 테이블에 모두 있고, 동등조인조건에 사용된
    -- 공통컬럼으로 조인결과에 두번 나오는 컬럼이기 때문에,
    -- 오라클에게 어느쪽 테이블에 있는 부서번호를 출력할지를
    -- 아래와 같이 컬럼의 모호성 제거 표기방식으로 알려줘야 합니다
    emp.department_id
FROM
    employees emp,          -- emp : 테이블 별칭(alias)
    departments dept        -- dept: 테이블 별칭(alias)
