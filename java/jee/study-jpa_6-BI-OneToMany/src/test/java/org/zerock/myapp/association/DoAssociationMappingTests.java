...
//	@Disabled
	...
	@Test
//	@RepeatedTest(1)
	@DisplayName("2. testObjectGraphTraverseFromTeamToMember")
	@Timeout(value = 3L, unit = TimeUnit.SECONDS)
	void testObjectGraphTraverseFromTeamToMember() {
		...
		//--2---
//		team1.getMembers().forEach(log::info); // log4j2
		team1.getMembers().forEach(m -> log.info(m.toString())); // slf4j
		team2.getMembers().forEach(m -> log.info(m.toString())); // slf4j
		team3.getMembers().forEach(m -> log.info(m.toString())); // slf4j
...
//	@RepeatedTest(1)
	...
	void testObjectGraphTraverseFromMemberToTeam() {
		...
		//--1---
		long [] memberPK = new Random().longs(5,1,21).distinct().limit(5).toArray();
		log.info("\t + memberPK : {} ", Arrays.toString(memberPK));
		
		//--2--- 
		LongStream.of(memberPK).forEach(seq ->{
