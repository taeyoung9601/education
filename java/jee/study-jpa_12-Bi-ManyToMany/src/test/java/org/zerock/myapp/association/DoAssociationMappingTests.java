...
//	@RepeatedTest(1)
	...
	void prepareData() {
		...
		Product2 product1 = this.em.<Product2>find(Product2.class, 1L);
		Product2 product2 = this.em.<Product2>find(Product2.class, 2L);
		Product2 product3 = this.em.<Product2>find(Product2.class, 3L);
		Product2 product4 = this.em.<Product2>find(Product2.class, 4L);
		Product2 product5 = this.em.<Product2>find(Product2.class, 5L);
		Product2 product6 = this.em.<Product2>find(Product2.class, 6L);
		Product2 product7 = this.em.<Product2>find(Product2.class, 7L);
...
//	@Disabled
	...
	@Test
//	@RepeatedTest(1)
	@DisplayName("2. testObjectGraphTraverseFromProductToShopper")
	@Timeout(value = 3L, unit = TimeUnit.SECONDS)
	void testObjectGraphTraverseFromProductToShopper() {
		log.debug("testObjectGraphTraverseFromProductToShopper() invoked.");
		
		 Long productId = new Random().nextLong(1L, 8L); // Half-open
	      
		 Product2 product = this.em.<Product2>find(Product2.class,productId);
		 Objects.requireNonNull(product);
      
		 product.getShoppers().forEach(p -> log.info(p.toString()));	
